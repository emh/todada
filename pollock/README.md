...
- **Layers** (☰) — Today, Work, Home, Ideas, Waiting, and Done are separate compositions. Each row shows a live mini-thumbnail of that layer's marks. Done collects every smeared mark; "Scrape off smeared marks" clears the history.
...
`localStorage` keys: `pollock.tasks`, `pollock.layer`, `pollock.ink`, `pollock.mood`, `pollock.seen` (first-run intro), `pollock.v` (schema version — bumping it wipes stale data on next load). The app starts as a blank canvas with an onboarding card explaining the gestures; clear the keys to reset it.

## Not implemented (from the mock)

Pinch-to-zoom into clusters, lasso clustering, and shake-to-scatter — all need multi-touch / device-motion APIs.
